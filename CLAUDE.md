...
## Lesson pages (from published articles)

Separate from the per-game pipeline above. The user uploads a PDF or web
export of a published chess article (a Novice Nook column, a magazine
piece) and asks for it as an interactive page. One article = one
self-contained file in `lessons/`, plus an entry in `lessons/index.html`.
These are written by hand — no tool generates them.

The rules that hold for every lesson page:

- **The article's text is reproduced verbatim and in full.** Nothing
  paraphrased, nothing summarised, nothing dropped. Filter out only the
  original page's furniture: site navigation, advertising, and
  "bestselling titles"-style sidebars. Keep the copyright/trademark line
  in the page footer.
- **Anything we add goes in an info panel**, styled as a GitHub-flavoured
  markdown alert (NOTE / TIP / IMPORTANT) and tagged "added to this page",
  so the author's words and ours never blur. Coaching advice, a mate the
  article only alludes to, a pattern worth naming — all of it belongs
  there, never inline in the article's prose.
- **Steps are numbered from 1**, with the material before the first
  diagram as step 0. Each step carries a position (FEN), the article's
  text for that step, and the hypothetical lines it discusses as playable
  SAN variations.
- **Diagrams are transcribed from the images** and must be verified: every
  variation the article gives has to be legal from the position you read
  off the picture. A line that will not parse means the position is wrong,
  not the article. Cross-check every reachable board state against
  python-chess before shipping.
- **Typography** follows the same rule as the game pages: literal Unicode
  characters, never HTML named entities.

### Cross-references between lessons

Articles in a series link to each other ("see *Hope Chess*"). Those
hyperlinks cannot be reproduced — the original URLs are unverifiable from
this sandbox, and the same "only verified URLs" rule as `drillLinks`
applies. Handle them with the **cross-reference registry** instead:

- Wrap each cross-reference in the article text as
  `<span class="xref">Exact Article Title</span>`.
- Each lesson page carries an `xrefs` map in its `LESSON` data block:
  article title → the lesson file in `lessons/` that covers it, as an href
  relative to the page (`"Hope Chess": "heisman-hope-chess.html"`).
- At render time the page looks up every `.xref` by its exact text. **A
  hit becomes a real link** to that lesson (solid underline). **A miss
  stays exactly as authored** — blue with a dotted underline, not
  clickable, carrying a tooltip that says no lesson page exists for it
  yet. That styling is deliberate and must be preserved: it tells the
  reader a link belongs there and simply has not been built.
- **Whenever a new lesson is added, wire up the cross-links in both
  directions**: add an entry to the new page's `xrefs` for every article
  it references that already has a page, and add an entry to every
  existing page whose `xrefs` comment lists the new article's title. Each
  page lists its own unresolved titles in that comment, so this is a
  matter of grepping `lessons/*.html` for the title.
- Never invent an external URL for a cross-reference, and never strip the
  `xref` styling to hide the fact that a target is missing.
...
- `lessons/` — interactive lesson pages built from published chess articles
  (one self-contained HTML file per article, plus `lessons/index.html`
  listing them, which has its own `LESSON LIST` marked region). Each page
  puts the article's text beside a board and splits it into numbered steps,
  with the article's diagrams as positions and its hypothetical lines
  playable. Written by hand, not generated by a tool, and unrelated to the
  per-game analysis pipeline above — see **Lesson pages (from published
  articles)** for the rules, including the cross-reference registry that
  links the lessons to each other.
