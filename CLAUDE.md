...
- `lessons/` — interactive lesson pages built from published chess articles
  ...
  listing them). Each page puts the article's text beside a board and splits
  it into numbered steps, with the article's diagrams as positions and its
  hypothetical lines playable. Source text is reproduced verbatim and in
  full; site navigation and advertising from the original page are left out,
  and anything added by us goes in a clearly marked info panel. These pages
  are written by hand, not generated by a tool, and are unrelated to the
  per-game analysis pipeline above.
